...
var usage = function() {
    ...
    console.log(" --insecure                  Don't validate SSL certs. Development only (also respects CLOUD_NO_VALIDATE env)")
    console.log(" --cloudport PORT            Use this alternate port instead of 443 when communicating to API (also respects CLOUD_PORT env)")
    console.log(" -v                          Verbose output for debugging")
}
...
// either one of these things will disable validation.
if(argv.insecure || (process.env.CLOUD_NO_VALIDATE && process.env.CLOUD_NO_VALIDATE=="true")) {
    console.log("WARNING: TLS validation is disabled!")
    process.env.NODE_TLS_REJECT_UNAUTHORIZED="0";
}

var cloud_port = undefined;

if(argv.cloudport || (process.env.CLOUD_PORT)) {
    var p = 0;
    if (argv.cloudport) p = argv.cloudport;
    else if (process.env.CLOUD_PORT) p = process.env.CLOUD_PORT;
    var p2 = parseInt(p);
    if ((p2 > 0) && (p2 < 65535) && (p2 != NaN)) {
        cloud_port = p2;
        console.log("NOTE: communicating with cloud services via port",cloud_port);
    } else {
        console.error("Invalid cloud port:",p)
        process.exit(1)
    }
}
...
//    "cloudURL": "https://cloud.wigwag.io",                         // ASK
    ...
    //Specify the devicedb cloud URL path
    "historyCloudURL": "https://history.wigwag.io",                   // ASK
...
var getCloudAPIURL = function(config) {
    if(cloud_port) {
        return util.format("%s:%d",config.cloudURL,cloud_port);
    } else {
...
var authedRequest = function(access_token, options, requestCB) {
    var opts = {};
    var headers = {
...
//        'accept': 'application/hal+json'
    ...
    verboseOut("OPTS: --------------> ", JSON.stringify(opts, null, 4))
    return request(opts, requestCB);
};

//var admin_access_token = null;

// new API auth
var tryAuth = function(creds,cookieJar) {
    return new Promise(function(resolve,reject){
        var url = getCloudAPIURL(config);
        console.log("   Authenticating to",url+"/oauth/access_token","...");
        request.post({
        ...
        if(err) {
            errorOut("Got error on request():",err);
            reject(err);
        } else {
            if(response.statusCode != 200) {
                console.error("Got back:",response.statusCode,body);
    ...
    //          console.log("got back:",body);
                if(body && body.access_token) {
                    okOut("Successful auth. ");
                    verboseOut('response body JSON:',util.inspect(response.body));
                    verboseOut('response headers-->',response.headers)
                    cookieJar.setCookie('access_token',body.access_token)
...
var pairRelay = function(user_access_token,relayID) {
	...
	return new Promise((resolve,reject) => {
		...
		authedRequest(user_access_token,{
            method: 'post',
		    url: url+'/api/relays/'+pairingCode+'/bindToAccount',
		    json: true,
//		    jar: cookieJar,
		    body: {
		    }
...
var getBatchFile = function(admin_access_token,prefix) {
// POST cloudURL+/apps/admin/relayBatch?config={"prefix":"WWSR","count":1,"url":null,"loading":true}
    ...
    return new Promise((resolve,reject) => {
	var url = getCloudAPIURL(config)+'/apps/admin/relayBatch?config='+encodeURIComponent(JSON.stringify(query));
	verboseOut("URL:",url);
    verboseOut("admin_access_token:",admin_access_token);
 	authedRequest(admin_access_token,{
        method: 'get',
        url: url,
//        headers: {Cookie:access_token,Accept:'application/json, text/plain, */*'},
	    json: true
//	    jar: cookieJar
	}, (err,response,body) => {
	    if(err) {
...
//	    console.log("   ...Successful auth.");
	    ...
	    return pairRelay(user_access_token,relay).then(function(){
...
.then(function(){
	...
	if(relay_config && relay_config[0]) {
        ...
        if(!config.historyCloudURL) config.historyCloudURL = relay_config[0].historyCloudURL;
	}
    ...
    if(!argv.cloud && !silent_mode) {
		...
		if(relay_config || !no_config_file_given) {
			...
			if(config.cloudURL != orig_cloud_url) { 
                ...
                config.historyCloudURL = base_url_proto + '://' + base_server+'-history' + base_domain_and_port;                                
			}
            ...
            var validURL = false;
            while(!validURL) {
                var url = rlSync.question("current history Cloud URL ["+config.historyCloudURL+"]:");
                if(url && url.length > 0) {
                    if(validCloudURL.test(url)) {
                        validURL = url;
                    } else {
                ...
                validURL = config.historyCloudURL;
                }
            }
            config.historyCloudURL = validURL;
...
.then(function(){
	...
	if(!silent_mode) {
        ...
        var getBatch = function(){
            ...
            return tryAuth(adminCreds,adminJar).then(function(admin_access_token){
                verboseOut("JAR>>>>>>>>",util.inspect(adminJar))
                return getBatchFile(admin_access_token,SOFT_RELAY_PREFIX,adminJar).then(function(obj){
...
//                                throw err;
    ...
    //                            resolve(obj);
                ...
                console.log("X -- Login failed. Retry.")                
...
var step3 = {};
step3.prom = new Promise(function(resolve,reject){
    step3.resolve = resolve;
    step3.reject = reject;
})
    

step3.prom.then(function(){
    console.log("Step #3.")
...
//		return true;
		if(config.cloudURL && relay_config[0].cloudURL != config.cloudURL) {
			warnOut("Note, cloud returned different 'cloudURL' in batch than from initial config (check this later):",relay_config[0].cloudURL);
		}
    }    
...
.then(function(){
    ...
    verboseOut("history URL:",config.historyCloudURL);
...
//    
        ...
        console.log("  (you may skip pairing by typing CTRL-C here. You can pair at");
        console.log("   any time by running the container with the 'pair' command.)");
	    var creds = getCreds();
