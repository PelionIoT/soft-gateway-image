...
function print_usage () {
    echo "Usage djs-soft-relay [-a HOST_STRING] [-c config] [-d] COMMAND [arguments ...]"
    ...
    echo "  -p PORT            Use PORT instead of default (443) when connecting"
    echo "                     services to the cloud"
    echo "  -a HOST_STRING     Add a DCS host to /etc/hosts"
    echo "            Example: -a \"127.0.0.1 dev.minikube\""
    echo "            This would add a dev.minkube, dev-devicejs.minikube, etc. host entries"
    echo "  -c CONFIG          Config file to use"
    echo "  -i                 Insecure. This is for when using soft-relay with a self-signed"
    echo "                     cert cluster" 
    echo ""
    ...
    echo ""
    echo "Advanced:"
    echo "  showvars           dumps meta vars, for debugging configs."
    echo "  info               will dump the pairing config file. Useful if you need"
    echo "                     to ascertain which soft-relay this is."
...
while getopts "ia:dc:p:" opt; do
    case $opt in
    i)  
        INSECURESSL="--insecure"
        export CLOUD_NO_VALIDATE="true"
        ;;
    a)  
        ADDHOST="${OPTARG}"
        ;;
    c)
        ...
        ;;
    p)
        export CLOUD_PORT="${OPTARG}"
        echo "Relay contact Cloud services via port ${CLOUD_PORT}"
        ;;
	d)
        VERBOSE_OPT="-v"
	    DEBUG_CONTAINER="1"
...
# make sure the devicedb storage directory is there.
if [ ! -d "/userdata/etc/devicejs/db" ]; then
...
# correct location in djs-soft-relay container for node.js is here:
if [ -z "${NODE_EXEC}" ]; then
    NODE_EXEC="node"
fi
...
if [ ! -z "${ADDHOST}" ]; then
    ...
    ${NODE_EXEC} /wigwag/modhosts.js "${ADDHOST}" >> /etc/hosts
    echo "/etc/hosts now:"
    cat /etc/hosts
    echo "done with hosts."
fi
...
#    chown -R wigwag:wigwag /wigwag
    ...
    if [ ! -e /config/${CONFIG} ]; then
        ...
        cd /wigwag/pairRelay
        echo "Running relay creation / pairing utility..."
        if [ -e "/config/${CONFIG}" ]; then
            ww-su-exec wigwag $NODE_EXEC pair.js "${VERBOSE_OPT}" "${INSECURESSL}" -c "/config/${CONFIG}"
        else
            ww-su-exec wigwag $NODE_EXEC pair.js "${VERBOSE_OPT}" "${INSECURESSL}" -c "/config/softRelaySetup.json"
        fi
        if [ $? -ne 0 ]; then
            echo "Pairing or setup failed."
            exit 1
        fi
    fi
...
# that setcap stuff is so node can bind to lower ports
echo "COMMAND: ${CMD}"
# this might fail, depending on the underlying file system type of docker
# (unionfs vs. overlayfs etc.)
...
# new way is maestro:
#    echo "    LD_LIBRARY_PATH=/wigwag/system/lib /wigwag/system/bin/maestro -config /wigwag/etc/softrelay-config.yaml"
...
if [ ${CMD} == "showvars" ]; then 
    LD_LIBRARY_PATH=/wigwag/system/lib /wigwag/system/bin/maestro -config /wigwag/etc/softrelay-config.yaml -dump_meta_vars
fi
...
if [ ${CMD} == "pair" ]; then
    ...
    ww-su-exec wigwag $NODE_EXEC ./pair.js "${VERBOSE_OPT}" "${INSECURESSL}" -c "/config/${CONFIG}" --justpair 
fi
