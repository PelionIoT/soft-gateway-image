...
apt-get --assume-yes install authbind
apt-get --assume-yes install psmisc          # killall command
...
ww-su-exec wigwag ${CONSOLIDATOR} -O /tmp/overrides.json -d dhclient *
ww-su-exec wigwag npm install
cd Runner
...
echo "*****************************"
echo "Installing wigwag-core-module npm modules..."
ww-su-exec wigwag ${CONSOLIDATOR} -O /tmp/overrides.json *
ww-su-exec wigwag npm install
remove_git_stuff
cd ${BASE_DIR}
